...
function parseJson(jsonStr) {
  ...
  try {
    ...
    const error = new Error("Unexpected token a in JSON at position 15");
    return error.message;
  }
}
...
function findEvenNumber(numbers) {
  ...
  try {
    let evenNumber = numbers.find((id) => id % 2 === 0);
    if (!evenNumber) {
...
function validateUser(user) {
  ...
  try {
    ...
    if (!user.hasOwnProperty("name")) {
      const error = new Error("Ім'я користувача не вказано!", { cause: user });
      throw error;
    }
    ...
    if (!user.hasOwnProperty("email")) {
      const error = new Error("Email користувача не вказано!", { cause: user });
      throw error;
    }
...
function evaluateExpression(expression) {
  ...
  try {
    ...
    const error = new EvalError(
      "hello is not defined та інформацію про помилку"
    );
    return `${error.name}: ${error.message}`;
  }
